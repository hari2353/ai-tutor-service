...
def pytest_configure(config):
    ...
    path = ROOT / which / "query_planner.py"
    spec = importlib.util.spec_from_file_location("query_planner", path)
    mod = importlib.util.module_from_spec(spec)
    sys.modules["query_planner"] = mod
...
@pytest.fixture
def R():
    return sys.modules["query_planner"]
