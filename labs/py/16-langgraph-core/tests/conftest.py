...
def pytest_configure(config):
    ...
    path = ROOT / which / "langgraph_core.py"
    spec = importlib.util.spec_from_file_location("langgraph_core", path)
    mod = importlib.util.module_from_spec(spec)
    sys.modules["langgraph_core"] = mod
...
@pytest.fixture
def R():
    return sys.modules["langgraph_core"]
