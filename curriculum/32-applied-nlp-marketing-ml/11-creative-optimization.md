...
### Q10 — Compare Thompson Sampling and UCB for creative allocation specifically, and state which you'd pick for a fast-moving, fatigue-prone creative portfolio.
**Testing:** whether the candidate can choose between bandit algorithms on the actual property that matters for this domain, not just name both.
**Answer:** UCB (Upper Confidence Bound) allocates deterministically based on an optimism bonus added to each arm's estimated mean, shrinking as an arm accumulates observations — principled and easy to reason about, but its confidence bounds are typically built assuming a stationary reward distribution, so it can be slow to react when a previously-strong creative's true performance starts declining, since the accumulated observation count keeps its confidence interval tight even as the underlying reward shifts. Thompson Sampling instead maintains a full posterior distribution per arm and samples from it to decide allocation, which naturally re-widens uncertainty when combined with a sliding-window or decayed-observation update (only recent observations feed the posterior), making it more naturally compatible with the non-stationary, fatigue-driven reward pattern this module already establishes as the actual shape of creative performance over time. For a fast-moving, fatigue-prone portfolio specifically, Thompson Sampling combined with decayed reward estimation is the better default; UCB is a fine choice for a more stable reward environment where creatives don't meaningfully decay within the test horizon.
**Follow-up trap:** *"Does Thompson Sampling's advantage here mean UCB is never appropriate for ad creative?"* — no — UCB remains a reasonable, simpler-to-implement choice when the portfolio is refreshed frequently enough (new creatives rotated in before fatigue meaningfully sets in) that non-stationarity isn't the dominant concern, or when the team wants UCB's more deterministic, easier-to-audit allocation behavior for stakeholder-facing reporting — the choice is about which property (non-stationarity handling vs. auditability/simplicity) matters more for the specific portfolio, not a universal ranking of one algorithm over the other.
