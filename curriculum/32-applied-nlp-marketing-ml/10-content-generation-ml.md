...
### Cost economics and hallucination-reduction numbers worth knowing

Token cost is a real architectural input, not an afterthought, at 22-locale generation volume. As of early 2026, frontier models used for higher-stakes generation run roughly $2.50-3 per million input tokens and around $15 per million output tokens (GPT-5.4, Claude Sonnet 4.6), while budget-tier models suitable for lower-risk, highly templated content (Gemini 1.5 Flash at $0.075/$0.30 per million tokens, GPT-4.1 Nano at $0.10/$0.40) run roughly 20-30x cheaper on the same volume — which is exactly why the risk-tiered architecture in this module doubles as a cost-tiered one: routing low-risk, high-volume categories (routine attribute refreshes) to a cheap model and reserving expensive frontier-model calls for flagship or claim-bearing copy is a real, quantifiable cost lever, not just a quality one. Two further production levers worth knowing cold: batch API processing (non-real-time, queued generation) commonly discounts 50% off standard per-token pricing, and prompt caching (reusing a stable system prompt/brand-voice-guide prefix across many generation calls) can cut effective input-token cost by up to 90% on the cached portion — both matter enormously at the call volume a 22-locale pipeline generates daily.

The hallucination-reduction case for RAG-constrained generation, quantified rather than asserted: published research finds retrieval-augmented grounding reduces hallucination rates by roughly 30-70% across domains compared to ungrounded generation, and improves factual accuracy by around 40% in aggregate. In structured-output tasks specifically (the shape most relevant to price/amenity/policy fields) — ungrounded generation shows hallucinated fields in roughly 21% of outputs, while retrieval-grounded generation drops that below 7.5% for step-like fields and below 4.5% for table-like structured fields; in one domain-specific study on medical consultation content, RAG eliminated hallucination entirely on the evaluated set (0% versus an 8% baseline rate for the ungrounded model). None of these figures transfer exactly to a marketing-copy context, and should be treated as evidence for the *direction and rough magnitude* of RAG's benefit, not a guarantee of a specific hallucination rate for any given pipeline — the only way to know your own system's actual rate is measuring it against your own claim-grounding gate's flagged-versus-caught outcomes, per Q3 and Q9 above.
...
- [LLM API Pricing Comparison In 2026: Every Major Model, Ranked By Cost — CloudZero](https://www.cloudzero.com/blog/llm-api-pricing-comparison/) — accessed 2026-08-08
- [Mitigating LLM Hallucinations through Domain-Grounded Tiered Retrieval — arXiv 2603.17872](https://arxiv.org/html/2603.17872v1) — accessed 2026-08-08
- [Reducing hallucination in structured outputs via Retrieval-Augmented Generation — arXiv 2404.08189](https://arxiv.org/pdf/2404.08189) — accessed 2026-08-08
