...
### Q10 — A company only operates in 12 total markets and wants to run a geo holdout, but 12 markets is too few to form a statistically comparable random control group by simple difference-in-differences. What's the alternative, and how does it work?
**Testing:** synthetic control as a real alternative to DiD when the number of available units is too small for naive randomization — a common real-world constraint the geo-holdout section doesn't fully resolve on its own.
**Answer:** Synthetic control constructs a weighted combination of the untreated markets specifically chosen so the weighted combination's pre-treatment outcome trajectory closely tracks the treated market's actual pre-treatment trajectory — rather than assuming any single untreated market or a simple average of all of them is comparable, it lets the data determine the combination that best reproduces the treated unit's own history. Post-treatment, the gap between the treated market's actual outcome and its synthetic counterfactual's projected outcome is the estimated incremental effect. This matters specifically at small N (a handful of markets) where simple DiD's parallel-trends assumption is hard to defend with only a few comparison units to check it against — synthetic control's pre-treatment fit is itself a direct, visual/quantitative check of how defensible the comparison is, rather than an assumption taken on faith.
**Follow-up trap:** *"What's the actual failure mode if none of the untreated markets, even weighted, can reproduce the treated market's pre-treatment trend well?"* — that's the honest signal the method doesn't apply here: a poor pre-treatment fit means no combination of available control markets is a credible counterfactual, and reporting a synthetic-control estimate anyway (with a bad fit) produces a confidently-stated number built on a comparison that doesn't hold up — the correct response is naming the fit quality explicitly and, if it's poor, falling back to a different design (a longer time-series MMM-based approach, or accepting the measurement question can't be answered causally with the markets available) rather than presenting a synthetic-control number as ground truth regardless of fit quality.
