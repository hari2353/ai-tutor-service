...
### Q8 — What does `__slots__` change about how instances are stored, and what does it cost you?
**Testing:** whether the candidate knows slots eliminate the per-instance `__dict__` and can attach a real number to the memory effect.
**Answer:** By default every instance carries a lazily-created `__dict__` holding its attributes — roughly 100+ bytes of dict overhead on top of the field values themselves (a 3-attribute object lands around 150 bytes total on 64-bit CPython, versus roughly 56-64 bytes with `__slots__`, though key-sharing dictionaries narrow that gap somewhat). With `__slots__`, attribute names become fixed descriptors storing values directly in the instance, so millions of small objects take a fraction of the memory — the reason it's standard in high-object-count code (parsers, game entities, record types). The costs: assigning an attribute not listed raises `AttributeError`, and instances get no weakref support unless `'__weakref__'` is added explicitly.
**Follow-up trap:** *"And `__slots__` makes attribute access dramatically faster too, right?"* — only marginally, and sometimes not measurably at all; the headline effect is memory, not speed, and candidates who lead with a claimed large speedup usually haven't benchmarked it — the honest answer is "primarily a memory optimization with fixed-shape semantics as the side effect."

### Q9 — Explain CPython's small-integer caching and string interning, and where relying on them breaks.
**Testing:** whether identity-vs-equality behavior is understood as an implementation detail with specific rules, not magic.
**Answer:** Integers −5 through 256 are preallocated singletons at interpreter startup, so any expression evaluating to e.g. `100` yields the same object and `x is y` holds, while two separately-computed `257`s are distinct objects that merely compare equal. Identifier-like string literals are interned at compile time (constants within a compiled unit share one object), so `'hello' is 'hello'` is typically `True`, whereas strings constructed at runtime (concatenation, `join`, slicing) generally produce fresh objects unless passed through `sys.intern()`. Both behaviors are documented CPython implementation details, not language guarantees.
**Follow-up trap:** *"So `is` is safe for comparing small ints and short strings since it always works there?"* — it "works" only by coincidence of the current implementation, then silently produces wrong results the moment a value crosses 256 or stops being literal-shaped (e.g. an integer parsed from user input); the rule interviewers want is `==` for values, `is` reserved for `None`/sentinel identity.

### Q10 — Why did CPython end up with one global interpreter lock rather than fine-grained locking around individual objects?
**Testing:** whether the GIL is understood as a deliberate engineering tradeoff tied to refcounting, not a historical accident.
**Answer:** Reference-count updates happen on virtually every operation — binding a variable, passing an argument, leaving a scope all touch `ob_refcnt` — so per-object locks would put synchronization overhead on essentially every bytecode instruction and drag lock-ordering/deadlock complexity into the interpreter core. One global lock makes the overwhelmingly common uncontended case nearly free, keeps the C implementation tractable, and preserves refcounting's deterministic immediate-deallocation semantics. The free-threaded build quantifies what replacing it costs: biased reference counting plus atomic fallback paths, running 5-10% slower single-threaded and using 15-20% more memory than the GIL build.
**Follow-up trap:** *"The GIL is just technical debt someone forgot to pay down?"* — that framing trips people into expecting removal to be a straightforward refactor; implementations without refcounting (Jython, GraalPy on the JVM's GC) never needed a GIL at all, which shows the lock is a direct structural consequence of CPython's memory-management design, not an oversight.
