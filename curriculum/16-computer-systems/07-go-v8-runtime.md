...
Full walkthroughs for both, including a `GODEBUG=schedtrace=1000` run showing real P/M/G counts and a Node.js `--trace-deopt` run showing an actual deoptimization event, live in `(lab pending)`.
...
### Q8 — Put real numbers on why 100,000 goroutines is routine while 100,000 OS threads is not.
**Testing:** whether the "goroutines are lightweight" claim reduces to arithmetic the candidate can actually perform.
**Answer:** A goroutine starts with a 2KB stack versus an OS thread's typical 1-8MB default, so 100,000 goroutines occupy roughly 200MB of stack memory against roughly 200GB+ for the same count of OS threads — unremarkable versus impossible on ordinary hardware. Creation cost differs the same way: spawning a goroutine is a user-space allocation plus a run-queue push (sub-microsecond), while thread creation crosses into the kernel to allocate scheduling structures and register with the scheduler. Contiguous stack growth (doubling via copy-and-pointer-rewrite) means each goroutine pays for call depth it actually uses rather than a fixed worst-case reservation.
**Follow-up trap:** *"So goroutines are effectively free — spawn one per item with no limit?"* — cheap is not free: each carries runtime metadata and scheduler pressure, and unbounded spawning under bursty load exhausts memory or downstream connection pools, which is exactly why bounded worker pools remain standard practice for high-throughput ingestion services.

### Q9 — Your Go service runs in a Kubernetes pod with a 2-CPU limit on a 64-core node. Before Go 1.25, what was wrong by default, and what symptom did it cause?
**Testing:** whether `GOMAXPROCS` is understood as a semantic knob with a known production failure mode, not just "number of cores."
**Answer:** Pre-1.25 Go defaulted `GOMAXPROCS` to the node's full core count — 64 Ps here — while the cgroup quota only permits 2 CPUs of actual runtime, so far more runnable work competes for the quota than it can serve and the kernel's CFS throttling kicks in repeatedly within each period. The signature symptom is inflated tail latency and throttling events despite deceptively low average CPU utilization graphs. Go 1.25 made `GOMAXPROCS` container-aware by default; on older versions the fix is setting it explicitly or using `uber-go/automaxprocs`.
**Follow-up trap:** *"So just always set GOMAXPROCS equal to the CPU limit?"* — that's the right default in most container cases but not a law: blocking syscalls keep extra Ms alive regardless of P count, and misreading what the limit represents leads to wrong numbers — the mature answer measures scheduling latency rather than applying the equation blindly.

### Q10 — What happens inside V8 when you `delete obj.x`, and why is deletion worse than adding properties?
**Testing:** whether shape-transition mechanics extend beyond "same order good" into deletion and dictionary mode.
**Answer:** Adding a property extends the hidden-class transition chain — V8 links a new Map describing the larger shape and may reallocate the property backing store — a bounded cost that consistently-shaped objects amortize well. Deleting breaks the shape model outright: V8 commonly transitions the object to dictionary mode (slow mode), abandoning hidden-class offsets so every subsequent property access becomes a hash-like lookup instead of a fixed-offset read. Any inline cache site that sees enough such irregular objects goes megamorphic, forfeiting TurboFan specialization there too — so one deleted property can degrade code that never touches that object directly.
**Follow-up trap:** *"`delete` is an ordinary language feature — surely it can't matter for performance?"* — it trips people because JS semantics treat delete as mundane key removal; the real cost isn't the removal itself but permanently demoting the object out of shape-tracking, which is why hot-path guidance is to assign `undefined` (preserving the shape) or build objects fully-formed rather than delete on them.
