...
### Q8 — Go's garbage collector is famous for prioritizing low pause times. What's the actual mechanism, and what does it cost you in exchange?
**Testing:** whether the candidate knows the concurrent tri-color mark mechanism specifically, not just "Go GC is fast."
**Answer:** Go uses a concurrent, tri-color mark-and-sweep collector that runs marking concurrently with the mutator (application goroutines) rather than stopping the world for the whole mark phase — objects are colored white (unvisited), grey (visited, children not yet scanned), or black (fully scanned), and the collector only needs a short stop-the-world pause at the very start (enabling write barriers) and briefly at the end (final termination check), not for the marking work itself. The cost: a write barrier — extra work on every pointer write during the concurrent mark phase, to catch the mutator concurrently mutating the object graph in a way that could hide a live object from the collector (the "tricolor invariant" violation) — which is real, always-on overhead during GC cycles, and the collector paces itself (via `GOGC`/`GOMEMLIMIT`) to trade memory headroom for GC frequency rather than optimizing for maximum throughput the way a stop-the-world collector focused purely on throughput could.
**Follow-up trap:** *"If GC pauses are already sub-millisecond in modern Go, why would you ever need to tune GOGC?"* — pause time being small doesn't mean GC *cost* is zero — a low `GOGC` value triggers collection more frequently (more total CPU spent on marking/write-barrier overhead, lower memory footprint), while a high value collects less often (less CPU overhead, higher peak memory) — tuning is about the memory-vs-CPU tradeoff, not pause time, which is why `GOGC`/`GOMEMLIMIT` tuning is still a real production lever even though Go's pauses were never the JVM-style multi-hundred-millisecond problem this tradeoff is sometimes assumed to be about.

### Q9 — Describe V8's actual compilation pipeline from parsed JavaScript to optimized machine code — name the stages, not just "V8 has a JIT."
**Testing:** the specific tiered pipeline (Ignition → Sparkplug → Maglev/TurboFan), the JS-runtime analog to the JVM's C1/C2 tiering covered in the previous module.
**Answer:** Ignition is V8's bytecode interpreter — every function starts here, executing quickly-generated bytecode with no compilation delay. Sparkplug (added 2021) is a fast baseline JIT that compiles Ignition bytecode almost directly to unoptimized machine code once a function runs at all, cutting interpreter dispatch overhead cheaply and almost immediately, without deep optimization. For genuinely hot functions, Maglev (a mid-tier optimizing compiler, added more recently to fill the gap Sparkplug's simplicity and TurboFan's slow compile time left) or TurboFan (V8's most aggressive optimizing compiler, using the type feedback from inline caches covered elsewhere in this module to generate speculatively-optimized machine code) take over, with deoptimization back to a lower tier when a speculative assumption breaks, mirroring the JVM's C2 deopt mechanism exactly.
**Follow-up trap:** *"Why did V8 add Sparkplug and Maglev as intermediate tiers instead of just tuning when TurboFan kicks in?"* — the same warmup-latency argument as the JVM's C1 tier: TurboFan's compile time is too slow to apply to every warm-but-not-hot function, and pure interpretation is too slow for functions that run enough to matter but not enough to justify TurboFan's cost — Sparkplug and Maglev fill the latency/throughput gap between the two extremes cheaply, the same structural reason C1 exists between the JVM's interpreter and C2, a genuinely convergent design across unrelated runtimes.

### Q10 — In Node.js, does an already-resolved Promise's `.then()` callback run before or after a `setTimeout(fn, 0)` scheduled earlier in the same tick? Explain the mechanism, not just the answer.
**Testing:** microtask vs macrotask queue ordering, one of the most commonly misdiagnosed sources of "unexpected" async ordering bugs.
**Answer:** The Promise callback runs first, always — V8 exposes a microtask queue (Promise `.then`/`.catch`/`.finally` callbacks, `queueMicrotask`) that Node's event loop (built on libuv) fully drains after every single callback it runs, before moving to the next macrotask (a `setTimeout` callback, an I/O completion callback, a `setImmediate` callback) even if that macrotask was scheduled earlier in wall-clock terms. This isn't a scheduling coincidence — it's a hard ordering guarantee: microtasks always run to exhaustion between any two macrotasks, regardless of the macrotask queue's contents or ordering.
**Follow-up trap:** *"A recursive chain of Promise.then() callbacks that keeps scheduling more microtasks — what happens to setTimeout callbacks and I/O during that time?"* — they starve, potentially indefinitely — since the microtask queue must fully drain before the event loop proceeds to the next macrotask phase, a microtask that keeps re-scheduling itself (a common bug pattern in recursive async code) can starve I/O callbacks and timers entirely, a genuine production incident pattern (an unresponsive Node process with 100% CPU and no visible stuck synchronous loop) that's specifically caused by microtask-queue starvation, not the classic single-long-synchronous-call event-loop-blocking pattern.
