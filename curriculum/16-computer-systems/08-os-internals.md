...
### Q8 — Why is reading the same file twice nearly free the second time, while `fsync()` after every write stays expensive every time?
**Testing:** whether the page cache is understood as the thing being measured in both directions — read acceleration and write-back cost.
**Answer:** The first read pulls file pages from storage into the kernel's page cache (milliseconds on spinning disks, tens-to-hundreds of microseconds on NVMe); the second read is a memcpy from RAM at nanosecond-to-microsecond scale with no storage involved — which is why a healthy Linux box reports most spare RAM as "buff/cache." Writes ride the same asymmetry: `write()` copies data into dirty page-cache pages and returns immediately, deferring physical write-back to kernel flusher threads. `fsync()` blocks until those dirty pages actually reach durable media — milliseconds even on NVMe, longer on HDD — and pays that cost every call, because durability, unlike caching, cannot be satisfied from RAM.
**Follow-up trap:** *"My write() returned, so the data is safely on disk?"* — it only reached volatile page cache: power loss or kernel panic loses it, which is precisely the gap fsync exists to close; the mirror-image mistake is benchmarking storage without dropping caches and measuring the page cache instead of the device.

### Q9 — How would you measure context-switch cost on real hardware instead of reciting textbook figures?
**Testing:** whether the candidate knows the ping-pong/lmbench methodology and its limits, not just the numbers.
**Answer:** The standard method is a ping-pong pair: two threads (then two processes) alternate handoff through a mutex/condvar or pipe for hundreds of thousands of iterations, and total elapsed time divided by twice the iteration count approximates per-switch cost including the scheduler wake path — lmbench's `lat_ctx` is the canonical tool, sweeping thread/process counts and working-set sizes to expose cache effects. Expect same-process thread switches in the low microseconds (roughly 1-4µs) and cross-process switches meaningfully higher, especially on hardware without PCID. Separating direct from indirect cost requires `perf stat`: elevated cache-miss rates during the run are the cold-cache tax, not the register save/restore itself.
**Follow-up trap:** *"Wall-clock time divided by iterations is the context-switch cost?"* — that quotient bundles futex wakeup latency, pipe overhead, and cache pollution with the mechanical switch, and published microsecond figures vary by kernel version, mitigation status, and methodology — the transferable knowledge is the order-of-magnitude relationship, not any single decimal.

### Q10 — `malloc(1GB)` returns instantly and barely moves RSS. What's going on, and what happens on the first write to each page?
**Testing:** whether lazy/demand-zero allocation is distinguished from eager commitment — the distinction behind trusting top/RSS and overcommit behavior.
**Answer:** The allocator serves large requests via mmap, reserving virtual address space without allocating physical frames; RSS grows only as pages are first touched. The first write to an untouched page takes a minor fault — roughly 1-10 microseconds — during which the kernel installs a page-table entry pointing at a freshly-grabbed zeroed frame (demand-zero paging). So a subsequent `memset` over the full 1GB incurs hundreds of thousands of faults plus real frame allocation, and naive benchmarks misattribute that entire cost to memset itself.
**Follow-up trap:** *"malloc returned non-NULL, so that memory is mine — guaranteed?"* — under Linux overcommit, the reservation is a promise the kernel can break: commit limits or the OOM killer resolve genuine shortfalls at fault time, not malloc time, which is exactly why memory footprints that "worked in staging" fall over in production.
