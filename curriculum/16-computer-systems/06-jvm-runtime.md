...
### Q8 — Show me code where escape analysis eliminates an allocation entirely, and explain exactly what the JIT does instead.
**Testing:** whether scalar replacement is understood as field-dissolution, not vague "the JIT is smart."
**Answer:** Take a `Point(x, y)` constructed inside a loop purely to compute `p.x - origin.x` and discarded — if C2 proves `Point` never escapes the method, it scalar-replaces the object: `p.x` and `p.y` become ordinary local/register values, eliminating the heap allocation, header write, and eventual young-gen collection cost for every iteration. One escaping path defeats the whole optimization — passing `p` to a logger call, storing it in a collection even on a branch never taken. Verify empirically with JMH's `-prof gc` showing a zero normalized allocation rate for the benchmark, not by reading source code.
**Follow-up trap:** *"So the object gets stack-allocated like a C++ local?"* — HotSpot doesn't literally place the object on the stack; it dissolves the object into its component scalars and allocates nothing anywhere — candidates who answer "stack allocation" have memorized the term without the mechanism.

### Q9 — Mechanically, how does G1 choose region sizes, and what does it actually do to honor its pause-time goal?
**Testing:** whether `MaxGCPauseMillis` is understood as an input to a sizing heuristic, not a promise.
**Answer:** At startup G1 picks a power-of-two region size between 1MB and 32MB, targeting roughly 2048 regions across the heap (overridable with `-XX:G1HeapRegionSize`). `-XX:MaxGCPauseMillis` (default around 200ms) feeds two heuristics: it sizes the young generation — smaller young regions mean fewer survivors to copy per collection, shorter pauses but more frequent collections — and it budgets how many old regions each mixed cycle evacuates per pause. If live-set size and allocation rate make the target arithmetically unreachable, G1 simply exceeds the goal and, under sustained pressure, escalates toward full GC rather than skipping required work.
**Follow-up trap:** *"Set MaxGCPauseMillis=10 and you're done — G1 will pause under 10ms?"* — it's a goal steering a heuristic, not a cap: demanding 10ms shrinks the young generation so aggressively that collection frequency explodes and throughput craters, which is why unrealistic pause goals are a recognized tuning anti-pattern.

### Q10 — An application server redeployed daily throws `OutOfMemoryError: Metaspace` after a couple of weeks. Walk through the mechanism.
**Testing:** whether classloader-leak pathology is understood end to end, from retention root to metaspace exhaustion.
**Answer:** Each redeploy instantiates a fresh webapp classloader holding newly loaded copies of every application class; the old loader would be collectable except something retains a reference chain to it — ThreadLocal entries in container-pooled threads, JDBC drivers registered on startup, static caches, shutdown hooks. That pinned loader keeps its entire loaded-class graph reachable, so metaspace climbs by roughly one application's class metadata per deploy until hitting `-XX:MaxMetaspaceSize` (or system memory) — linear growth per redeploy is the diagnostic signature. Confirm with a heap dump after several redeploys: duplicate classes differing only by defining loader, traced back to the GC root pinning each stale loader.
**Follow-up trap:** *"Raise MaxMetaspaceSize and ship it?"* — growth here is linear in deploy count, so any ceiling only postpones the outage; the fix is severing the retention chain (clearing ThreadLocals, deregistering drivers, or deploying classloader-leak-prevention filters), and proposing only the bump signals missing the leak entirely.
