...
### Q8 — Your service adopts virtual threads (Project Loom, JEP 444) for a blocking-I/O-heavy workload and throughput improves, but one specific code path shows no improvement and even regresses under load. What's the likely cause?
**Testing:** carrier-thread pinning, the sharpest real gotcha in virtual-thread adoption.
**Answer:** Virtual threads are cheap because many of them multiplex onto a small pool of platform "carrier" threads, unmounting from the carrier whenever they block so the carrier can run a different virtual thread — but a virtual thread executing inside a `synchronized` block (or a native/JNI frame) cannot unmount, because the JVM can't safely migrate monitor ownership across carrier threads mid-hold. That specific code path "pins" its carrier thread for the duration of the blocking call inside the `synchronized` section, so under load, many virtual threads blocked inside synchronized code starve the limited carrier pool exactly the way platform threads would have — silently reverting to platform-thread-like scaling limits for that one path while the rest of the service scales fine.
**Follow-up trap:** *"What's the concrete fix, without a full rewrite of that code path?"* — replace `synchronized` with `java.util.concurrent.locks.ReentrantLock` around the blocking call specifically (`ReentrantLock` doesn't pin, since it's not implemented via a JVM monitor the same way), reserving the rewrite for the specific pinning hot spot rather than the whole codebase; JDK tooling (`-Djdk.tracePinnedThreads=full` or JFR's virtual thread pinning events) is the mechanical way to actually locate which `synchronized` blocks are pinning in production before deciding where to spend the rewrite effort.

### Q9 — A long-running service's Metaspace usage grows steadily across redeploys until it hits `-XX:MaxMetaspaceSize` and throws `OutOfMemoryError: Metaspace`. What's the likely cause, and how does this differ from a heap leak?
**Testing:** classloader-leak diagnosis, a real and distinct failure mode from ordinary object-reference heap leaks.
**Answer:** Metaspace holds class metadata (method bytecode, constant pools, the classes themselves), and a class can only be unloaded once its defining classloader becomes entirely unreachable — a classloader leak (an application server redeploying a webapp without every reference to its old classloader being released — a static field in a shared library holding a reference, a live thread started by the old webapp still running, a JDBC driver registered globally and never deregistered) means the old classloader, and every class it loaded, stays permanently resident, growing Metaspace by roughly one webapp's worth of class metadata per redeploy. This differs from a heap leak in kind, not just location: a heap leak is live *objects* accumulating; a classloader leak is live *classes* accumulating, and no amount of heap tuning touches it since Metaspace is a separate memory region entirely.
**Follow-up trap:** *"How do you actually confirm it's a classloader leak versus a genuinely growing class count from dynamic class generation (e.g. heavy proxy/bytecode-generation frameworks)?"* — take heap dumps across two redeploys and diff the count of classloader instances specifically (a tool like Eclipse MAT has a dedicated classloader-leak report) — a classloader count that grows by roughly one per redeploy and never drops is the leak signature; a framework generating many classes but properly releasing old ones on redeploy shows class *churn*, not classloader *accumulation*, and the fix in that case is tuning generation patterns, not chasing a leak that isn't there.

### Q10 — What does the Java Memory Model's "happens-before" relationship actually guarantee, and why is `volatile` not a substitute for `synchronized` in every case?
**Testing:** precise understanding of the JMM beyond "volatile makes things visible across threads."
**Answer:** Happens-before is a partial ordering the JMM guarantees the compiler/JIT/CPU cannot reorder across in ways that would violate it — a write happens-before a subsequent read of the same variable by another thread specifically when there's a synchronizing action connecting them (a `volatile` write happens-before a subsequent `volatile` read of the same field; a monitor unlock happens-before a subsequent lock of the same monitor). `volatile` gives visibility and ordering for reads/writes of that single field, but it provides no mutual exclusion — two threads can both read a `volatile` field, compute something, and write back, racing on the compound operation exactly like the CPython `x += 1` race, because `volatile` never made the read-modify-write atomic, only each individual read and write visible.
**Follow-up trap:** *"So when is plain `volatile` actually sufficient, without `synchronized` or an atomic class?"* — specifically for a single field where every operation on it is a pure read or a pure write, never a read-modify-write depending on the field's current value — a status flag one thread sets and others poll is a correct `volatile` use case; a counter incremented by multiple threads is not, and needs `AtomicInteger` (compare-and-swap under the hood) or `synchronized`, because the JMM's happens-before guarantee for `volatile` says nothing about the atomicity of a multi-step operation on that field.
