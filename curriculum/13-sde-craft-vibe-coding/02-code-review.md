...
The mechanical difference between feedback that sticks and feedback that gets grudgingly applied once and forgotten: naming the specific mechanism of future cost ("this will need a matching change in the retry handler next time someone touches timeout behavior, because the two are now implicitly coupled through this shared constant") versus asserting a preference ("I'd structure this differently"). The first gives the author (and anyone reading the review later) a transferable piece of judgment they can apply to their *next* PR without this reviewer present; the second only produces compliance on *this* PR, if that, and often produces quiet resentment because a preference framed as a requirement is, correctly, arguable. A useful self-check before posting a review comment: could I state, in one sentence, what specifically breaks and under what future condition — if not, the comment is a preference and should be phrased as "consider," not "please change."

### Receiving review well at senior/principal level

The failure mode at this level isn't usually defensiveness in the crude sense (arguing loudly) — it's a subtler one: treating every comment on a design you own as requiring a defense, rather than triaging first whether the comment identifies a real, stated cost or is a preference you're free to accept or decline. The actual skill is the triage, done quickly and out loud: "is there a concrete cost named here, and is it one that compounds (gets more expensive over time) or is one-time" — a compounding cost is worth addressing even under deadline pressure, because the two-day fix now is cheaper than paying the cost repeatedly; a one-time cost is a legitimate tradeoff to accept and ship. Distinguishing a real objection from a style preference is genuinely hard to do about your own work in the moment, which is exactly why "does this comment name a specific, concrete cost" is a mechanical test worth applying deliberately rather than trusting your in-the-moment emotional read of whether a comment feels fair.

### What the research actually says about review size and latency (know these numbers)

The mechanical layer this module deliberately doesn't dwell on (covered fully in `T27-pr-review`) still has real, citable numbers worth knowing cold, because "review smaller diffs, review them promptly" is exactly the kind of advice that lands better backed by a real study than asserted as taste. SmartBear's peer-review study of a Cisco Systems team — one of the largest published peer-review datasets — found reviewers should look at no more than 200-400 lines of code (LOC) at a time; a 200-400 LOC review conducted over 60-90 minutes yields roughly 70-90% defect discovery (of 10 real defects present, a properly-conducted review of that size finds 7-9), and defect density found drops significantly once the inspection rate exceeds about 500 LOC/hour — reviewing faster doesn't save time, it just means the same 60-90 minutes finds fewer of the bugs that are actually there. The same study found review effectiveness drops off after about 60 minutes of continuous review in one sitting, and that lightweight, tool-assisted review finds just as many defects as formal inspection meetings while taking less than 20% of the time (formal/heavyweight inspection averages roughly 9 hours per 200 LOC and requires up to 6 participants in a room). Separately, spot-checking only 20-33% of a codebase's changes was found to produce measurably lower defect density than reviewing nothing, at a fraction of full-coverage review's time cost — a real data point for teams weighing partial review coverage against reviewer bandwidth.

Google's own published data (Sadowski et al., *Modern Code Review: A Case Study at Google*, 2018) is the other reference point worth knowing: change lists under 100 lines have a median review turnaround under 1 hour at Google, while change lists over 1,000 lines take 24+ hours — and splitting a large change into a stack of small CLs measured 15-20% less total reviewer time than reviewing the equivalent single large CL, not just faster wall-clock turnaround. Google's median review latency across all change sizes is under 4 hours, well ahead of the industry norms the same research compares it against. None of this changes the principal-level judgment calls this module is actually about — but citing real numbers instead of "keep PRs small" as an assertion is itself a credibility signal in an interview, and it's a concrete lever a principal reviewer can push on organizationally (PR-size norms, review SLAs) independent of any individual review's content.
...
- [Best Practices for Code Review — SmartBear (Cisco Systems peer-review study)](https://smartbear.com/learn/code-review/best-practices-for-peer-code-review/) — accessed 2026-08-08
- [Modern Code Review: A Case Study at Google — Sadowski, Söderberg, et al., ICSE 2018](https://sback.it/publications/icse2018seip.pdf) — accessed 2026-08-08
