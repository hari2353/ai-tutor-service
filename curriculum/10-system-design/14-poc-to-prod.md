...
### Q9 — Leadership wants a single up-front cost/time number for "the project," but you know the whole point of staged gates is that later stages are conditional on earlier ones passing. How do you reconcile these?
**Testing:** whether the candidate can translate a probabilistic, gated process into the kind of commitment executives actually need, without either lying with false precision or refusing to give a number.
**Answer:** Give a number for the next gate only, with an explicit range (not a false-precision point estimate) for what happens after, conditioned on that gate's outcome — "POC is 1 week, ~$15k in engineering time; if it clears, prototype is 2 weeks and ~$30k; if MVP doesn't hit its adoption bar, the project stops there rather than continuing to production" is a real commitment leadership can plan around, and it makes the conditional structure visible instead of hiding it inside one big number that implicitly assumes every gate passes. Naming the abandonment branch explicitly is what separates this from a waterfall estimate wearing gate vocabulary.
**Follow-up trap:** *"Finance needs one number for this quarter's budget, not four conditional ones."* — give the sum of all four as a worst-case ceiling *and* the POC-only number as the actual near-term ask, explicitly labeled as two different things — collapsing them into one number either overstates near-term spend or hides the fact that 33.8% of projects like this are abandoned before production, which is exactly the information finance needs to not be surprised later.

### Q10 — Production rollout reveals the MVP's core adoption assumption was wrong for the general population, not just a quirk of the pilot cohort. Do you roll back to prototype, or push forward with production hardening?
**Testing:** judgment about when a later-stage signal should reopen an earlier gate rather than being absorbed as a production bug.
**Answer:** This is a signal about the *prototype's* question (is this the right thing to build), discovered late — the fact that it surfaced during production rollout rather than at the prototype gate doesn't change which question it answers. Pushing forward with production hardening treats a product-fit problem as an operational one, which doesn't fix it; the honest move is to treat this as the prototype gate failing retroactively and go back to real user validation before investing further in scaling something that doesn't fit, even though that's organizationally painful after production money has already been spent. The sunk cost of the MVP/production work is a reason people resist this, not a reason it's wrong.
**Follow-up trap:** *"Isn't 'go back to prototype' after we're already in production an admission the whole gate process failed?"* — no — it means the MVP pilot cohort wasn't representative (a sampling problem, covered in Q5), not that gates as a concept don't work; the correct fix is re-running prototype-stage validation with a more representative sample before deciding whether to invest further, not abandoning staged validation because one cohort selection was flawed.
