...
### Q9 — What do you deliberately throw away when moving from POC to production, and why is throwing away cheaper than keeping?
**Testing:** whether they treat disposability as a designed property of early-stage code rather than sloppiness to be forgiven.
**Answer:** Deliberately discarded: hardcoded credentials and paths, notebook scaffolding, single-user no-auth assumptions, sample-data shortcuts (the ten hand-picked queries), and the missing error taxonomy — everything the POC was built to *not* care about. Throwing away is cheaper because those omissions are embedded as invisible assumptions in the code's structure; unwinding them in place costs more in archaeology and regression risk than rebuilding the narrow proven core with real boundaries. What survives is not code but learning — measured latencies, the eval question set, configuration knowledge, and the exit-criterion evidence itself.
**Follow-up trap:** *"Can't you keep the core algorithm and just wrap production services around it?"* — sometimes, for a genuinely thin slice, but "just wrap it" reliably accretes into the eighteen-months-later hardcoded-key scenario this module describes; the defensible middle is naming in advance exactly which artifacts survive (benchmarks, eval sets) versus which code gets rewritten, so "keep the core" is an explicit decision rather than drift.

### Q10 — Where does load testing sit in your gate structure, and what has to be true before a load test counts as passed?
**Testing:** whether load testing is a real gate with evidence standards or a checkbox run late.
**Answer:** It becomes meaningful at the MVP→production transition — before that there's no representative backend for it to measure. Passed means driven at projected peak with a headroom multiplier (not just average), with realistic traffic shape (bursts, skew, concurrency), against production-shaped dependencies rather than mocks, with failure injection (dependency slow or down) alongside volume, and producing a documented capacity result: holds X rps at p99 under Y ms, saturates at Z with bottleneck W identified. A test that only proves "the happy path is fast at low concurrency" hasn't retired the scale risk this gate exists to retire.
**Follow-up trap:** *"We load-tested and hit the target RPS — why would you still block the gate?"* — hitting a target number answers only the question that was asked; if the test used uniform traffic, mocked dependencies, or no failure injection, it never exercised the failure modes that dominate real incidents, so the production-gate question ("will this hold up under real load and real failure") remains unanswered despite green numbers.

### Q11 — What are your concrete on-call readiness criteria before a system starts taking production pages?
**Testing:** whether they operationalize the ownership/runbook dimension instead of treating it as organizational trivia.
**Answer:** The bar is concrete: alerts routed to a named rotation (not a channel nobody watches) with thresholds tied to user-facing SLOs rather than machine vitals; a runbook per alerting condition linking detection query → likely causes → first actions → escalation path; someone other than the author having actually executed that runbook end-to-end in a game-day drill; rollback tested recently enough that whoever is on call trusts it; and a kill switch or feature flag as the fastest containment path. If any of those are missing, the system isn't production-ready regardless of how clean the deploy was — it's an MVP wearing production's pager.
**Follow-up trap:** *"Isn't requiring a game-day drill before every launch just process theater?"* — the criterion is demonstrated behavior, not ceremony: an unexecuted runbook is how on-call discovers mid-incident that the documented query doesn't exist; scoping the drill to the top three paging conditions keeps the cost proportionate while preserving the point.
