...
### Q8 — Why did the circuit breakers and health checks that AWS's and Cloudflare's downstream customers had in place not save them during the October/November 2025 outages?
**Testing:** whether the candidate understands resilience patterns operate at a layer that can itself be undermined by the layer below.
**Answer:** A circuit breaker protects a caller from a *slow or erroring* dependency by tripping open after a failure threshold — but it still needs to be able to reach the dependency (or a health-check endpoint) to make that decision, and both the DynamoDB DNS corruption and the Cloudflare proxy crash-loop broke the addressing/discovery layer itself, not just the application logic behind it. If your breaker's health check resolves the same broken DNS name, or your fallback path routes through the same crashed proxy fleet, the breaker degrades gracefully into "also broken" rather than "correctly open." Resilience patterns assume the failure is contained to the thing being protected against; a control-plane failure violates that assumption by taking down the protection mechanism and the protected path together.
**Follow-up trap:** *"So what's the actual mitigation, since breakers don't help here?"* — genuine redundancy at a layer *above* the shared control plane: a second DNS resolution path, a secondary CDN/edge provider, or a fallback that doesn't depend on the same infrastructure being healthy — which is expensive and often not justified, so the honest staff-level answer names the cost/benefit tradeoff rather than claiming breakers alone are sufficient for this failure class.

### Q9 — You're asked to retrofit blast-radius containment into an existing global control plane without a full rewrite. What's the minimum viable first step?
**Testing:** practical, incremental thinking versus a hand-wavy "redesign it" answer.
**Answer:** Start with propagation staging, not architecture change: introduce a canary tier (a small percentage of fleet or one low-traffic region) that receives every control-plane push first, gated behind automated health signals (error rate, crash rate, latency) with an explicit bake time before wider rollout, and an automated rollback that's faster than full propagation. This doesn't require redesigning the control plane's consistency model — it just inserts a checkpoint before "broadcast to everyone," which is exactly what both AWS and Cloudflare cited as their actual post-incident remediation rather than a ground-up rearchitecture.
**Follow-up trap:** *"What if the bad config is a security-critical WAF rule that needs to propagate in seconds, not the 10+ minutes your staged rollout takes?"* — maintain two paths: a default staged path for routine changes, and a narrower, more tightly-scrutinized fast path for genuinely urgent changes (smaller validation, not zero validation) — collapsing back to a single always-fast path to avoid this complexity is exactly the design that produced the Cloudflare incident.

### Q10 — Staff-level: why did detailed, technical public postmortems (rather than vague "some users were affected" statements) become the industry norm, and what does that norm's existence tell you about how to run your own incident reviews?
**Testing:** whether the candidate connects the case studies' existence as teaching material to the cultural practice that produced them, and can apply that internally.
**Answer:** Google's SRE book (2016) formalized "blameless postmortem" — the incentive structure before it punished honesty (admitting a mistake risked blame, so postmortems stayed vague), and blameless framing made it safe to publish the actual mechanical chain of failure, which is precisely the level of detail that makes a postmortem *useful to someone who wasn't there*, including future interview candidates studying it years later. The internal-process lesson: a postmortem that assigns blame to a person optimizes for that person being more careful next time (which doesn't scale and doesn't transfer); a postmortem that treats the failure as a systems/process gap (no verification step, no staged rollout, no quorum) produces a fix that prevents the *entire class* of failure, not just a repeat of the exact same incident.
**Follow-up trap:** *"Isn't 'blameless' just a way to avoid accountability?"* — no, and the distinction matters: blameless means not punishing the individual who happened to be holding the pager or wrote the line of code, while still being rigorous about identifying exactly what process or system allowed that individual action to cause the outage — accountability shifts from "whose fault" to "which safeguard was missing," which is a stricter standard, not a softer one, because it requires fixing the gap rather than just remembering to be careful.
