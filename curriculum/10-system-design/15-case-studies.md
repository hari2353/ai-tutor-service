...
### Q8 — Your organization writes a postmortem after every incident, yet the same class of outage keeps recurring. What's broken about the process?
**Testing:** whether the candidate understands postmortems as a process with its own failure modes, not a document ritual.
**Answer:** Recurrence almost always traces to the remediation half, not the analysis half: action items without owners or deadlines, fixes scoped to the exact trigger rather than the class ("add a retry for this endpoint" instead of "verify post-change propagation for all control-plane changes"), and no tracking loop that surfaces when items silently stall. The second failure mode is blame creeping back in — once people fear consequences, writeups soften toward "human error," which hides the systemic gap; Knight Capital's silent-deploy flaw was a process gap, and a blaming postmortem would have stopped at "the release manager should have checked." The fix is mechanical: every postmortem produces owned, dated action items tracked to completion, and the review question is "could this class recur elsewhere," never "who pushed the button."
**Follow-up trap:** *"Doesn't 'blameless' just mean nobody is ever accountable?"* — no: blameless separates accountability for the system (a named owner closes the deployment-verification gap) from punishment of individuals, because punished people stop supplying the honest detail the analysis runs on — defensive writeups degrade detection quality long before anyone notices, which is exactly how the same outage class survives three postmortems.

### Q9 — You're leading a major schema-and-backfill migration. Walk me through your rollback strategy, and why it's categorically harder than code rollback.
**Testing:** whether they understand that data migrations can't simply be reversed the way deployments can.
**Answer:** Code rollback is redeploying the previous artifact; migration rollback has to contend with data written *under the new shape* after cutover, which a down-migration script cannot reconstruct. The workable pattern stages things so rollback never needs to undo data: expand-migrate-contract (add the new shape alongside the old, backfill, switch readers, remove the old only later), dual-writing during the transition window, and isolating every destructive change (drops, type narrowing) into a final separately-reversible step. Rollback readiness is defined before starting: a restore path actually tested against production-scale data with a known recovery-time objective, and an explicit decision rule for the point past which abort is no longer cheap and forward-fix becomes the only option. GitHub's 2018 split-brain is the adjacent cautionary case — recovery cost was dominated not by the trigger but by reconciling divergent writes afterward.
**Follow-up trap:** *"We keep the down-migration scripts ready and test them in staging — isn't that sufficient?"* — no: a down-migration restores schema, not the writes accepted against the new shape between cutover and rollback; testing it in staging on static data proves nothing about hours of live-traffic divergence in production, which is precisely why the strategy must make rollback a non-event (the old shape was never removed) rather than a reconstruction project.

### Q10 — Defend this record: your capacity plan passed load testing and the service still fell over on launch day. What did the load test miss?
**Testing:** whether they can diagnose a failed capacity defense with specifics instead of "we underestimated."
**Answer:** The usual suspects are shape, dependencies, and amplification. The load test drove uniform traffic at target RPS while launch traffic arrived bursty and skewed, so instantaneous peak demand exceeded anything tested. The test saturated only the service under test while the real bottleneck was a shared dependency — connection pools, the database, a downstream API — that behaves fine at tested levels and queues catastrophically above them. And retries amplified the overload, converting partial slowdown into a retry storm. The credible defense names which resource saturated first, why the test's traffic model never exercised it, and the fix: load-test the dependency chain at realistic traffic mixes, add load shedding and bulkheads so overload degrades instead of collapsing, and tune autoscaling on the binding constraint — scaling CPU when connections were the limit protects nothing.
**Follow-up trap:** *"So the lesson is just provision more headroom everywhere?"* — blanket headroom is expensive and still fails if the binding constraint was misidentified; the senior version is targeted: instrument which pool saturates first, protect that one specifically, accept measured risk elsewhere — overprovisioning everything converts a capacity incident into a permanently inflated bill without removing the failure mode.
