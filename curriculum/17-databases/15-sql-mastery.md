...
### Q9 — You're handed a gaps-and-islands problem live that you've never seen before. Talk me through your approach before writing any SQL.
**Testing:** whether they can derive the pattern from first principles or freeze without the memorized variant.
**Answer:** Four questions in order: what's the partition (per user, per account), what's the ordering column, what does "consecutive/belongs-together" mean concretely in that column's units, and what defines a boundary. From there it's mechanical: number rows within each partition with `ROW_NUMBER()`, compute ordering-value minus row-number-times-step — constant inside any unbroken run, changed at every gap — then `GROUP BY` that island key. Before trusting the result, hand-trace a five-row toy dataset containing one gap and one tie, because the technique silently assumes exactly one row per ordering value (dedupe first if not).
**Follow-up trap:** *"What if there is no regular step — event IDs with irregular gaps, say?"* — then the constant-difference trick doesn't apply, and the honest pivot is the sessionization formulation instead: compare each row to the previous one (`LAG`), flag a boundary wherever the gap exceeds the domain's definition of "consecutive," and turn flags into group ids with a running sum — recognizing which of the two island formulations fits is the actual skill being probed.

### Q10 — Precisely when does a recursive CTE terminate, and what's your guarantee against a cycle in dirty production data?
**Testing:** execution-model depth beyond "it walks a tree."
**Answer:** Natural termination is when an iteration's recursive member returns zero new rows — a leaf was reached or no further matches exist. On cyclic data that never happens, so the guarantee must be structural: an explicit depth cap (`WHERE depth < N`) in the recursive member, or genuine cycle detection — carrying the visited path and stopping when the current node reappears, or PostgreSQL 14+'s `CYCLE` clause doing exactly that. The subtlety worth stating: switching `UNION ALL` to `UNION` only prunes rows byte-identical to prior output; any cycle whose rows evolve (an incrementing depth column, a growing path string) keeps producing novel rows forever, so `UNION` alone is not a termination guarantee.
**Follow-up trap:** *"Isn't the depth cap arbitrary — what if legitimate data is deeper than N?"* — it is arbitrary, deliberately: pick N comfortably above the domain's plausible maximum (an org chart deeper than ~50 levels is corrupted data by definition), log when the cap trips, and treat a tripped cap as a data-quality signal rather than silently truncating — the alternative, no cap, is a runaway query eating the connection pool, which is strictly worse.

### Q11 — Beyond top-N-per-group, where does LATERAL actually earn its place, and what can't you express without it?
**Testing:** whether they know the pattern's genuine scope or reach for it reflexively.
**Answer:** Three jobs a window function can't do: invoking a set-returning function per row (`jsonb_array_elements`, `unnest` on a per-row array column); a correlated lookup with its own ORDER BY and LIMIT ("best match per customer" under per-row conditions); and `LEFT JOIN LATERAL (... LIMIT 1)` as an optional-best-match-or-NULL join, where a plain JOIN would flatten into inner-join semantics. It also replaces stacks of scalar correlated subqueries with one subquery returning several computed columns per row — one pass instead of N.
**Follow-up trap:** *"Couldn't every LATERAL be rewritten as a window function over the whole table?"* — the top-N case often can, but per-row set-returning functions fundamentally can't: no single table-level computation expands each row's private array differently; claiming otherwise signals pattern-matching rather than understanding, and the follow-through is checking which form the planner handles better on real data distribution via `EXPLAIN ANALYZE` rather than assuming.
