...
### Adjacent landscape: LlamaIndex, Haystack, Semantic Kernel, Dify

Four names adjacent to the six above come up in interviews often enough to warrant positioning, and the trap is evaluating them on this module's axes when they mostly optimize different ones.

**LlamaIndex** is data/index-centric where LangChain is chain-centric: its core primitives are indexes, retrievers, query engines, and ingestion pipelines, not agent loops. It wins decisively for fast document-QA prototypes — a working RAG pipeline over a folder of PDFs is a handful of lines with sensible defaults — and fights you once the requirement is custom control flow: stateful branching, approval gates, resumable loops are LangGraph's territory, and bending LlamaIndex toward them means abandoning its defaults. The mature pattern isn't picking a winner, it's embedding LlamaIndex retrieval components inside a LangGraph loop. **Haystack 2.x** rebuilt its entire API around explicit component pipelines — typed components connected by declared inputs and outputs — making it the most DAG-like of the RAG frameworks and a solid fit for search-shaped products, though its agent story lags everything dedicated on this page. **Semantic Kernel** was Microsoft's .NET-first enterprise entrant (planners, memory, first-class C# support), whose distinct pull was always the enterprise/.NET install base rather than technical differentiation — which is precisely why it ended up merged into Microsoft Agent Framework at GA rather than continuing solo, per the lineage above. **Dify** and **Flowise** are visual builders: drag-and-drop workflow canvases that genuinely accelerate prototypes and stakeholder demos, and cap out the moment you need custom tools, nontrivial shared state, or version-controlled branching beyond what the canvas exposes — teams routinely prototype in them and rebuild in code, which is a legitimate lifecycle if it's said out loud up front. One adjacent term worth having cold: managed vector databases (Pinecone et al.) carry their own convenience-versus-portability-and-cost tradeoff, covered properly in `T17-vector-db-compare`, not here.
