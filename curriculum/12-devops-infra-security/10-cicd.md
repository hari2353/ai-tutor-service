...
### Q9 — GitOps means your desired state lives in Git, but production needs real secrets (DB passwords, API keys). How do you reconcile "everything in Git" with "don't put plaintext secrets in Git"?
**Testing:** whether the candidate knows the actual mechanisms, not just "use a secrets manager" hand-waving.
**Answer:** Two real patterns, both keeping Git as the source of truth for *intent* without storing plaintext: (1) Sealed Secrets (Bitnami) encrypts the secret client-side with a cluster-specific public key before committing, and only the in-cluster controller holding the matching private key can decrypt it — so the committed artifact is safe even if the repo is public. (2) External Secrets Operator instead stores only a *reference* in Git (which key, in which vault) and syncs the actual value from an external secrets manager (Vault, AWS Secrets Manager) into a cluster Secret at reconcile time — Git never holds the value at all, encrypted or not. The second is more common in mature setups because it centralizes rotation and audit in the secrets manager rather than the cluster.
**Follow-up trap:** *"External Secrets Operator means the cluster now needs credentials to reach the external vault. Haven't you just moved the bootstrapping problem, not solved it?"* — yes, and say so plainly: you still need one bootstrap credential (ideally short-lived, workload-identity-based — IRSA on EKS, Workload Identity on GKE — not a static key) for the operator itself to authenticate to the vault; the win isn't eliminating a credential, it's shrinking the number of places a long-lived secret can leak from N committed-and-encrypted blobs to one centrally-rotated, centrally-audited store.

### Q10 — Your org wants to guarantee that only images actually built by your CI pipeline (not a manually pushed image with the same tag) can be deployed. What's the mechanism?
**Testing:** supply-chain security awareness — image signing and admission-time verification, a genuinely current staff-level topic.
**Answer:** Sign the image at build time (cosign, keyless signing via Sigstore/Fulcio using the CI workflow's OIDC identity as the signing identity, so there's no long-lived signing key to manage) and attach the signature and provenance attestation (SLSA-style, recording which workflow/commit/repo produced it) to the image in the registry. At deploy time, an admission controller (Kyverno or the equivalent policy engine) verifies the signature and the expected signing identity before allowing the pod to schedule — a manually pushed image, even with an identical tag, fails verification because it was never signed by the CI workflow's OIDC identity.
**Follow-up trap:** *"Tags are mutable — someone can retag a legitimate signed image to point at malicious content after the fact. Does signing protect against that?"* — no, and this is the actual trap: signature verification must pin to the image *digest* (immutable content hash), not the tag, and your `Rollout`/`Deployment` manifests in Git should reference images by digest for anything security-sensitive — verifying a signature against a mutable tag only proves *some* image that tag once pointed to was signed, not that the tag still points there now.
