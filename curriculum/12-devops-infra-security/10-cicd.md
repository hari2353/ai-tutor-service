...
### Q9 — Walk me through what actually gates an automated rollback in your canary setup — which metric, what threshold, and what evidence proves it works?
**Testing:** real progressive-delivery evidence — whether an automated rollback has actually fired correctly, not just "we use Argo Rollouts."
...
**Follow-up trap:** *"So a global error-rate threshold catches every bad deploy?"* — no: low-traffic routes dilute the aggregate ratio below detectability, wrong business logic returning 200 OK never moves an error metric at all, and small weights produce noisy samples that flap the rollout in the opposite direction — which is exactly why route-scoped queries, absolute request-count floors, and widened promote/rollback thresholds exist.

### Q10 — What are the ways GitOps drift reconciliation itself fails, misleads you, or causes incidents?
**Testing:** failure modes of `selfHeal`/`ignoreDifferences`-class machinery beyond "drift gets fixed."
**Answer:** Three recurring ones: fields legitimately mutated by another in-cluster actor (an HPA rewriting replicas, a mutating webhook injecting sidecars) produce permanent `OutOfSync` noise until declared in `ignoreDifferences`; `selfHeal: true` reverts a legitimate emergency hotfix someone applied via `kubectl edit` mid-incident within one reconcile cycle (~3 minutes); and `Synced` + `Healthy` describes resources matching Git and passing resource-level probes — neither says anything about application behavior, which is why metric gating lives outside the GitOps controller.
**Follow-up trap:** *"ArgoCD shows Synced and Healthy — so the deploy is good, right?"* — that's precisely the lie: sync status only compares manifests to Git and health checks only probe resource conditions, so a perfectly-synced deployment serving 500s at 100% of traffic shows green in the UI; treating controller status as an SLO substitute is the failure mode.
...
## Changelog
- 2026-08-23 — added Bitbucket Pipelines vs GitHub Actions vs Jenkins comparison to "How it actually works"
- 2026-08-03 — created
