...
### Q10 — You're the staff engineer asked to fix a distributed monolith spanning 12 services, with no room for a big-bang rewrite. What's your sequencing?
**Testing:** migration strategy under real constraints — incremental de-risking versus a rewrite that will get cancelled halfway.
**Answer:** Start by mapping which tables are read/written cross-service today (credential audit, query logs) — that map *is* the actual coupling graph, not the service diagram. Pick the single highest-traffic or highest-incident-cost violation and fix ownership there first: introduce a read API or CDC-derived replica, cut the second writer over, verify with a reconciliation job running in parallel for a release or two before removing the old path. Repeat table by table rather than service by service, because ownership is the unit of coupling, not the service boundary. Only after ownership is clean do you revisit whether the *service* boundaries themselves are still wrong — some pairs will turn out to need merging, others will turn out to be fine once they're not sharing tables. This is a strangler-fig migration applied to data ownership rather than to code.
**Follow-up trap:** *"Six months in, leadership asks why deploys still aren't independent."* — say plainly that ownership fixes remove the *silent* coupling but versioned-contract discipline on the remaining synchronous calls is a separate, ongoing fix; independent deploys require both, and claiming ownership alone solves it is the trap.

### Q11 — You introduce CDC (e.g. Debezium) org-wide to kill dual writes. What new failure mode have you just created?
**Testing:** whether the candidate treats CDC/outbox as a tradeoff, not a free win — staff-level awareness that every fix has a new failure surface.
**Answer:** You've traded an application-level atomicity bug for an infrastructure-level single point of coupling: every downstream consumer now implicitly depends on the source database's write-ahead-log format and replication behavior, so a schema change upstream (a column rename, a type change) can silently break every CDC consumer at once, and replication lag becomes a new source of read-your-own-write bugs consumers didn't have before. You've also added an operational dependency — the CDC connector itself is now a thing that pages, needs monitoring for lag and dead-letter handling, and is a new single point of failure for every service that reads from it.
**Follow-up trap:** *"So is CDC net worse than dual writes?"* — no, but say why precisely: dual writes fail *silently* and *randomly* (undetectable drift), while CDC failures are *visible* (lag metrics, connector health, DLQ depth) and *centralized* (one thing to monitor instead of N ad-hoc reconciliation jobs) — trading an invisible many-sited failure mode for a visible, centrally-owned one is the actual win, not "CDC never breaks."
