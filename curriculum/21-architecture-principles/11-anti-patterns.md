...
### Q10 — Teams adopt microservices deliberately, with good intentions, and still end up with a distributed monolith. How does that happen despite the intent, and where do you intervene?
**Testing:** whether they can explain the accretion mechanics rather than blaming bad up-front design.
**Answer:** Intent doesn't survive contact with three accretion mechanisms. The org chart didn't change along with the topology (Conway's Law), so seams follow reporting lines and a "temporary" shared database introduced during the migration never gets unbonded. Early contracts go unversioned because one team initially owns both sides of every call, so coupling stays invisible until a second team inherits half of it. And services get split along technical layers rather than business capabilities, so every real business change touches several services at once. The intervention points, in leverage order: exclusive data ownership enforced at the credential level (which kills the shared-table coupling mechanically), versioned contracts with explicit deprecation windows, and — the step people skip — merging services back together if they're still coupled after both fixes, since the boundary itself was wrong.
**Follow-up trap:** *"So the fix is a stricter approval gate before anyone creates a new service?"* — creation-time governance misses the actual mechanism: distributed monoliths form *after* creation, through convenience table access and synchronous calls added one PR at a time; CI-level detection (database-access audits, deploy-order analysis, call-graph checks) attacks the accretion process, whereas an approval gate only polices the one moment when everything still looks clean.

### Q11 — Your search index and your database have started quietly disagreeing in production. How do you detect dual-write drift systematically, and how do you distinguish it from ordinary replication lag?
**Testing:** operational detection instincts versus reciting the outbox pattern as a buzzword.
**Answer:** Detection is a reconciliation job: sample or stream keys from both stores, compare field-by-field, and track drift count over time — rising or non-converging drift is the signal. The discriminator between dual-write divergence and lag is behavior over time: replication lag is bounded and self-healing (the index catches up once write pressure drops, and each disagreement carries timestamps within the expected convergence window), while true dual-write divergence is permanent — rows written during a crash-between-writes never converge on their own and need manual repair or replay. The structural fix is outbox/CDC so there's exactly one atomic write and the derived stream can't desynchronize; the reconciliation job then shifts from firefighting tool to continuous invariant check.
**Follow-up trap:** *"Just diff the row counts between database and index periodically, right?"* — count comparison false-alarms constantly on legitimate lag and soft-deletion windows while missing the worse failure mode entirely: same-count-different-content rows, which is precisely what crash-between-writes produces; keyed comparison within a staleness bound is the minimum viable check, and even that is detection, not prevention.
