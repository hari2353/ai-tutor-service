...
### Q10 — A product team wants to ship a large parallax-scrolling animation on the marketing homepage. What accessibility concern applies here that has nothing to do with screen readers or keyboard navigation?
**Testing:** whether the candidate's accessibility model extends beyond the blind/screen-reader-user and keyboard-user cases most interview prep over-indexes on, to vestibular and motion-sensitivity disorders.
**Answer:** Large-scale motion, parallax effects, and auto-playing animation can trigger genuine physiological symptoms (dizziness, nausea, disorientation) for users with vestibular disorders — this isn't a preference issue, it's an accessibility barrier in the same category as insufficient contrast or missing alt text, just affecting a different population. The mechanism: respect the `prefers-reduced-motion` media query, checking whether the user's OS-level setting requests reduced motion, and provide a substantially reduced or static alternative (remove parallax offset, replace auto-playing motion with a static image or a user-triggered animation) rather than merely slowing the animation down, since even slow large-scale motion can trigger symptoms for sensitive users.
**Follow-up trap:** *"Does `prefers-reduced-motion: reduce` mean removing all animation entirely?"* — no, and over-applying it is itself a UX regression — small, functional motion (a button's hover state, a focus-ring transition, a loading spinner) is generally fine and often *helps* orientation; the WCAG guidance specifically targets large-scale, parallax-style, or auto-playing motion that affects a significant portion of the viewport, not micro-interactions — the fix is scoped to the actual trigger category, not a blanket "disable all CSS transitions" response.
