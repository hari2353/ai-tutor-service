...
### Q10 — A component measures a DOM element's size in an effect and uses that measurement to immediately reposition another element. Using `useEffect` for this produces a visible flicker; switching to `useLayoutEffect` fixes it. Why, mechanically?
**Testing:** the precise timing difference between the two hooks, a common source of a real, visible bug rather than an abstract distinction.
**Answer:** `useEffect` runs asynchronously *after* the browser has painted the current frame to the screen — so if the effect measures the DOM and then triggers a state update that changes layout, the user briefly sees the pre-update layout painted, then a second, corrected paint after the effect runs, producing a visible flicker. `useLayoutEffect` runs synchronously *after* React has updated the DOM but *before* the browser paints — so a measurement-and-adjustment cycle inside it completes and is reflected in the DOM before anything is shown to the user at all, producing a single, correct paint with no intermediate flash.
**Follow-up trap:** *"If useLayoutEffect avoids flicker, why isn't it the default instead of useEffect?"* — because it's synchronous and blocks the browser from painting until it finishes, so using it for anything that doesn't need to complete before paint (data fetching, subscriptions, logging, anything not directly repositioning/resizing something the user is about to see) adds unnecessary blocking work to the critical rendering path — `useEffect` is the correct default for the large majority of effects; `useLayoutEffect` is reserved specifically for the narrow case of synchronous DOM measurement-and-mutation that must be invisible to the user.
