...
### Q10 — War story: a list of rows with uncontrolled text inputs rendered with index-as-key. A user deletes row 2 of 3, and the text they typed in row 2 now appears where row 3 was. Trace exactly what happened.
**Testing:** whether the candidate can narrate reconciliation mechanics through a realistic bug, not just recite "never use index as key."
**Answer:** Items [A,B,C] rendered with keys 0,1,2; after deleting B the array is [A,C] with keys 0,1. React's diff matches by key: position 0 updates in place, position 1 updates in place (now displaying C's props), position 2 unmounts — meaning the DOM node that physically held row B is reused to show C. Props re-render correctly (labels say the right thing), but the uncontrolled input's DOM state — the text typed into B — stays attached to that reused node, so B's typed text now sits in C's row. Keying by stable `item.id` makes the diff see key-B removed and key-C moved, so C's node travels with its state intact.
**Follow-up trap:** *"But React re-rendered the correct label text — how can the state be wrong at the same time?"* — props always come from the fresh render pass, while uncontrolled DOM state lives in the node itself and follows whatever node key-matching decides to reuse; that split (correct props, orphaned DOM state) is exactly why the bug looks impossible until you trace the key matching step by step.
