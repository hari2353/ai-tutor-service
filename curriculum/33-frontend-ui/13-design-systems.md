...
### Q9 — Describe the three token layers (primitive → semantic → component), what each is for, and how theming swaps without forking a single component.
**Testing:** whether the candidate can articulate the full layering including the component tier, not just semantic-vs-primitive.
**Answer:** Primitives are raw, context-free values (`color.blue.500`, `spacing.4`) — a palette that nothing outside the token system references. Semantics assign intent (`color.background.primary`, `color.text.danger`) and map to primitives differently per theme — light maps `background.primary` to gray.50, dark to gray.900 — so activating dark mode remaps only that layer and every component updates with zero code changes. Component tokens bind one component slot to a semantic (`--button-primary-bg → {color.action.primary}`), letting consumers restyle a specific component tier without touching global semantics or forking the component.
**Follow-up trap:** *"Why bother with a component-token tier at all — can't components just read semantics directly?"* — they can, and most should; the component tier earns its keep only when a specific component needs locally overridable binding (per-brand button styling) without polluting global semantics with component-specific names — adding that tier everywhere is speculative cost, like any premature abstraction layer.

### Q10 — Telemetry shows 11 of 40 consuming teams still use `Button`'s `ghost` variant, which design has decided to remove. Walk through your release process.
**Testing:** staff-level: whether the candidate has an actual deprecation/breaking-change policy for shared UI, not just "bump the major version."
**Answer:** Ship it behind a major version with a published deprecation window: first release adds a dev-mode runtime warning plus an ESLint rule flagging `variant="ghost"` usage so teams get an exact inventory of their call sites, alongside a migration guide and a codemod mapping `ghost` to the nearest surviving variant. Announce an end-of-life date (commonly two release cycles or ~90 days), run visual-regression across the whole system before the removal lands, and keep the changelog entry explicit about what breaks. Only then does the major version drop the variant.
**Follow-up trap:** *"Isn't a changelog entry plus a major bump enough?"* — no: consuming product teams are usually on their own release cadence and don't watch your changelog, and unlike a broken function signature a removed variant may fail silently (fallback styling can look fine) — silent visual drift across 11 products is exactly the blast-radius problem semver-on-code-API alone cannot surface.
