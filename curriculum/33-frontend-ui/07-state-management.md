...
### Q10 — URL-as-state has real costs too. What are the failure modes of putting too much state in the URL, and where do you draw the line?
**Testing:** whether the candidate can argue both directions on URL state, not just "filters belong in the URL."
**Answer:** History pollution: pushing a history entry on every keystroke of a filter input buries the user's back-navigation — continuous input should `replace()` while discrete changes (tab switch, page number) `push()`. Size limits: URLs past roughly 2,000 characters risk truncation across browsers and tooling, and servers commonly reject request-header lines above ~8KB, so elaborate multi-facet filter state can simply stop working. Exposure: anything in the query string leaks into browser history, `Referer` headers, and server/CDN access logs — tokens, PII, or email addresses in URLs are a recurring real-world incident class. The line: encode shareable, restorable view configuration (filters, tabs, pagination); keep high-frequency, sensitive, or deeply structured state elsewhere.
**Follow-up trap:** *"Fine — we'll base64-encode the whole filter object into one compact param. Solved?"* — encoding shrinks characters, not information: it's still in logs and history, still hits the same ceilings at scale, and now the URL is opaque — users can't read or hand-edit it, which kills half the reason it was in the URL at all.
