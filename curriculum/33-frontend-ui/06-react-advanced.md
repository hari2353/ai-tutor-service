...
### Q10 — A Server Component passes a prop to a Client Component that includes a `Date` object, a `Map`, and a callback function. What happens, and why?
**Testing:** whether the candidate understands the RSC serialization boundary as a real, enforced constraint, not just an abstract "server and client are different" statement.
**Answer:** Props crossing from a Server Component to a Client Component must survive serialization into the RSC payload format, which supports a specific, bounded set of types — plain objects, arrays, strings, numbers, booleans, and a few special-cased types the framework explicitly handles, but not arbitrary class instances or functions by default. A `Date` typically serializes but deserializes as something requiring explicit handling depending on the framework version (React itself doesn't automatically reconstruct it as a `Date` instance without the framework's help); a `Map` generally is not directly serializable and needs manual conversion (e.g., to an array of entries) before crossing the boundary; a plain callback function cannot cross at all — passing one produces a runtime error, because a function is server-side code and code doesn't travel across the RSC payload boundary the way data does. The one specific, framework-supported exception is a **Server Action** (a function specifically marked with `"use server"`), which the framework serializes as a reference the client can invoke via a special RPC-like mechanism, not as literal function code shipped to the browser.
**Follow-up trap:** *"If a plain function can't cross the boundary, how does an `onClick` handler ever get attached to something a Server Component rendered?"* — it doesn't happen inside the Server Component at all — the interactive element with the `onClick` handler must itself be (or be inside) a Client Component, which defines its own handler using its own client-side code; a Server Component can render a Client Component and pass it *data* props, but the actual event-handling function has to originate on the client side of the boundary, not be authored server-side and shipped across as a prop.
