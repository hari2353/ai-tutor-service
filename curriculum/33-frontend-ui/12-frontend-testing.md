...
### Q9 — A team relies heavily on Jest snapshot tests for their component library. What's the specific failure mode that makes large snapshots low-value over time, and when are snapshots actually the right tool?
**Testing:** whether the candidate can distinguish snapshot testing's genuine use case from its most common misuse.
**Answer:** A large, full-component-tree snapshot changes on almost any legitimate change to the component (a new class name, reordered props, a wrapper div added for an unrelated reason), and reviewers facing a wall of diff noise on every PR develop the habit of running `--updateSnapshot` and approving without actually reading what changed — at that point the snapshot has stopped catching real regressions, since the human verification step it depends on has been trained out of the team by its own noisiness. Snapshots earn their keep specifically for small, stable, low-churn output (a utility function's formatted output, a specific serialized data structure) where a diff is genuinely rare and therefore genuinely meaningful when it appears — not as a default testing strategy for entire component render trees that change shape routinely as part of normal development.
**Follow-up trap:** *"If snapshot diffs are getting rubber-stamped, isn't the fix just 'tell reviewers to read them more carefully'?"* — that's a process fix asking humans to overcome a tool's structural failure mode through vigilance alone, which doesn't scale — the more durable fix is shrinking what's actually snapshotted (a specific, narrow piece of output rather than the whole render tree) so a diff is rare enough to warrant real attention, or replacing broad component snapshots with RTL-style behavioral assertions that fail only when something a user would actually notice changes.

### Q10 — A codebase reports 95% line coverage but still ships regressions regularly. What does line coverage actually measure, and what would give a more reliable signal?
**Testing:** whether the candidate understands coverage-percentage as a necessary-but-insufficient metric, and knows a concrete stronger alternative.
**Answer:** Line coverage only confirms a line was *executed* during the test run — it says nothing about whether the test actually asserted on the consequence of that line running correctly, so a test that calls a function and checks nothing about its result still counts as "covering" every line inside it. Mutation testing gives a materially stronger signal: a tool (Stryker for JS/TS) systematically introduces small deliberate bugs ("mutants" — flipping a `>` to `>=`, changing a `+` to `-`, negating a boolean) into the source, reruns the test suite against each mutant, and reports the percentage of mutants that were actually *caught* (a test failed) versus *survived* (every test still passed despite the introduced bug) — a high line-coverage percentage with a low mutation-kill percentage is the precise, measurable signature of tests that execute code without meaningfully verifying it.
**Follow-up trap:** *"Should a team just switch from line coverage to mutation-testing percentage as their primary CI gate?"* — mutation testing is significantly more computationally expensive (rerunning the full suite once per mutant, potentially thousands of mutants for a large codebase) and slower to run, so most teams use it as a periodic, targeted audit (run against a specific critical module, or on a slower nightly/weekly cadence) rather than a per-PR blocking gate the way line coverage thresholds often are — treating it as a wholesale replacement for a fast per-PR signal misunderstands the tradeoff between signal quality and feedback speed.
