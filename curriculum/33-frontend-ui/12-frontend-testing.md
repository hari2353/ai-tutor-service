...
### Q9 — Everyone agrees E2E suites rot. What specific practices keep one alive and trusted for years instead?
**Testing:** staff-level: whether the candidate has maintained a suite long-term, not just diagnosed decay after the fact.
**Answer:** Keep the suite small enough to stay fast — a hard budget (under ~10 minutes, parallelized across CI workers) forces marginal coverage down to the integration layer where tests are cheaper and less flaky. Enforce a quarantine SLA: any test failing twice without a corresponding product bug leaves the merge-gating set within 24 hours, so flakiness never normalizes into "just re-run it." Isolate data per run (unique accounts/records created and cleaned up per test) to kill cross-test interference, and stick to role/label locators so routine copy changes don't break selectors.
**Follow-up trap:** *"Playwright supports automatic retries — shouldn't we just retry failing tests to hide the flakes?"* — retries mask the signal you need: a test that passes on attempt 2 is still reporting nondeterminism, and retry-until-green is precisely the habit that turns a trusted suite into noise people stop gating merges on — treat retries as a diagnostic flag, never a fix.

### Q10 — You support Chromium, Firefox, and WebKit across desktop and mobile viewports. How do you manage visual-regression baselines without the matrix exploding?
**Testing:** whether the candidate has operated visual regression at real scale, not just heard of screenshot diffs.
**Answer:** Full baselines everywhere multiply fast — 3 engines × 2–3 viewports × hundreds of snapshots is thousands of images to review per intentional change — so mature setups designate one canonical engine+viewport (typically Chromium at 1280px) as the primary merge gate and treat the rest as targeted spot-checks on visually risky surfaces. Services like Chromatic/Percy absorb the matrix (per-browser baselines captured, but diffs batched into a single review approval), while self-hosted Playwright `toHaveScreenshot()` needs Docker-pinned, per-engine baselines kept in the repo or object storage. Masking of dynamic content and forced `prefers-reduced-motion` apply identically at every point in the matrix.
**Follow-up trap:** *"Why not let developers regenerate baselines on their own machines for faster feedback?"* — font rasterization and anti-aliasing differ by OS/GPU, so a MacBook-generated baseline produces spurious pixel diffs against Linux CI indefinitely — local runs may preview, but only containerized captures may update stored baselines, or every PR fights phantom regressions.
