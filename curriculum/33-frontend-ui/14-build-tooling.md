...
### Q9 — Walk through the full chain that turns a minified production stack trace back into readable original source, including which source-map variant you'd generate and why.
**Testing:** whether the candidate knows the source-map-to-error-tracker pipeline operationally, not just that maps exist.
**Answer:** The production build emits a `.map` file (generated→original positions encoded as base64 VLQ in the `mappings` field); the error-tracking SDK captures the minified stack from the browser, and an authenticated CI step uploads the map to Sentry/Datadog, which symbolicates frames back to original `file:line:column` plus original symbol names. Production typically uses the `hidden` variant — a full-fidelity external map with no `//# sourceMappingURL` comment in the public bundle — so debugging data exists for the tracker without exposing source to anyone opening devtools. Cheaper modes (`eval`-based dev builds, `cheap` variants that drop column accuracy) trade fidelity for build speed and are fine locally but degrade production trace precision.
**Follow-up trap:** *"We configured hidden source maps — why are Sentry traces still minified?"* — generating the map accomplishes nothing by itself: if the CI upload step is missing, or uploads are tagged with a release/commit identifier that the SDK's event tagging doesn't match, symbolication silently never fires — the artifact exists but nothing links it to incoming events.

### Q10 — You wrote pure ESM imports everywhere, yet the bundle still contains dead exports. What CommonJS-related causes explain this even with zero third-party dependencies?
**Testing:** whether the candidate understands CJS interop failures inside their own toolchain, not just "CJS packages don't tree-shake."
**Answer:** Suspect your own transpilation first: a Babel/TypeScript config emitting CommonJS (a stray `"modules": "commonjs"`, or CJS output inherited from a shared config package) converts your static `import`/`export` into dynamic `require`/`exports` before the bundler ever sees them, and a dynamic exports object can't be statically pruned. Second, a mixed graph: a single `require()` anywhere in the chain (app code importing a CJS config/helper) marks that module and everything reachable through its exports object as unshakable. Third, interop wrappers: bundlers synthesize namespace objects for default-imports of CJS whose property access isn't statically provable, blocking export-level elimination even in otherwise-ESM code.
**Follow-up trap:** *"Our tests all pass through Vitest, so the module graph is obviously fine — why does this matter?"* — test runners routinely transform ESM to CJS themselves (Jest historically required it), so green tests say nothing about whether the production bundler receives genuine ESM; the only real verification is inspecting the built output (grep the bundle for the supposedly-dead export), not passing test suites.
