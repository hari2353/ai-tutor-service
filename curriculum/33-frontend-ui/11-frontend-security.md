...
A concrete, checkable exercise: stand up a minimal Express server serving a page with a deliberately vulnerable `dangerouslySetInnerHTML` rendering unsanitized query-string content, confirm the XSS actually fires (an `<img src=x onerror=alert(1)>` payload in the URL), then add a strict nonce-based CSP header and confirm the same payload no longer executes (the injected markup renders but the browser refuses to run the embedded handler since it's not associated with the trusted nonce) — then fix the actual sink with DOMPurify as the primary defense, leaving the CSP in place as defense in depth. This sequence is valuable specifically because it makes visible that CSP and input sanitization are two independent layers, not redundant ones. Reference: `(lab pending)`.
...
### Q9 — When is a CSP built on `'sha256-...'` hashes the pragmatic choice instead of nonces, and what operational realities make nonce-based CSP harder to deploy than it sounds?
**Testing:** whether the candidate has actually deployed strict CSP, not just read that nonces beat host allowlists.
**Answer:** Nonces require injecting a fresh random value into both the response header and every `<script>` tag at render time, which rules out purely static hosting (CDN-served HTML has no per-response logic) and conflicts with caching — if the CDN serves one cached HTML document to many users, they all share a single nonce, defeating per-response freshness. Hashes pin exact script content (`script-src 'sha256-<base64>'`), so they work on fully static sites with zero server-side logic, at the cost of recomputing hashes at build time whenever script content changes. A common real-world split: hashes for a static marketing site, nonces with `strict-dynamic` for the server-rendered app behind controlled caching.
**Follow-up trap:** *"So we'll generate the nonce once per deploy instead of per response — close enough?"* — that collapses the scheme back to a static secret: anyone who fetches the page once holds a nonce valid for every user until the next deploy and can inject scripts carrying it — per-response cryptographic randomness is the entire security property, and "per-deploy" quietly discards it.

### Q10 — Name concrete npm supply-chain defenses beyond "run npm audit," explaining the mechanism of each.
**Testing:** staff-level: whether the candidate knows enforcement mechanisms, not a checklist of tool names.
**Answer:** Lockfiles pin resolved versions, but only `npm ci` enforces them — plain `npm install` still updates the lockfile within semver ranges, letting a freshly published malicious patch version slip in, so CI should fail on any lockfile diff. Install scripts are the primary malware execution vector (Shai-Hulud spread via lifecycle hooks), and `ignore-scripts=true` (allowlisting packages that legitimately need scripts) stops untrusted postinstall code from running at all. Provenance attestations (`npm publish --provenance`, Sigstore-backed) cryptographically bind a published tarball to its source repository and CI run, letting you verify a package was built by its claimed GitHub Actions workflow rather than an attacker's machine.
**Follow-up trap:** *"`npm audit` reports zero vulnerabilities — are we covered?"* — no: audit matches against the known-advisory database, and a brand-new malicious publish (exactly the September 2025 chalk/debug pattern, 2.6 billion weekly downloads affected) has no advisory yet; behavioral scanners and script/provenance controls cover the novel-package case CVE matching structurally cannot.
